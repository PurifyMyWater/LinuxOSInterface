...
uint32_t LinuxUntypedQueue::size() { return maxMessages_; }
...
bool LinuxUntypedQueue::isEmpty() { return length() == 0; }

bool LinuxUntypedQueue::isFull() { return length() >= maxMessages_; }
...
bool LinuxUntypedQueue::receive(void* message, uint32_t maxTimeToWait_ms)
{
    ...
    const ssize_t result =
        mq_timedreceive(mqd_, static_cast<char*>(message), messageSize_, nullptr, &ts);
